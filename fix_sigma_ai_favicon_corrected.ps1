...
foreach ($file in $htmlFiles) {
    ...
    try {
        ...
        # 3. Limpiar Sigma AI existente para reemplazarlo con la version funcional
        # Usar regex con opciones para manejar multilinea
        $content = [regex]::Replace($content, '<div id="sigma-logo"[^>]*>.*?</div>', '', [System.Text.RegularExpressions.RegexOptions]::Singleline)
        $content = [regex]::Replace($content, '<div id="sigma-chatbox"[^>]*>.*?</div>', '', [System.Text.RegularExpressions.RegexOptions]::Singleline)
        
        # Remover JavaScript existente de Sigma AI
...
# Mostrar tabla detallada (solo primeros 20 para no saturar)
Write-Host "`nDETALLE (primeros 20 archivos):" -ForegroundColor Yellow
...
Write-Host "Archivos completamente actualizados: $completeCount" -ForegroundColor Green
Write-Host "Archivos con problemas: $incompleteCount" -ForegroundColor Yellow
