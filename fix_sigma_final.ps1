# Script final para arreglar Sigma AI y agregar favicon
# Version corregida usando [regex]::Replace
# Autor: Sigma Games

Write-Host "Iniciando reparacion final de Sigma AI y favicon..." -ForegroundColor Green
...
foreach ($file in $htmlFiles) {
    ...
    try {
        ...
        # 3. LIMPIEZA COMPLETA usando [regex]::Replace
        # Remover divs de sigma
        $content = [regex]::Replace($content, '<div id="sigma-[^"]*"[^>]*>.*?</div>', '', [System.Text.RegularExpressions.RegexOptions]::Singleline)
        
        # Remover scripts que contengan sigma
        $content = [regex]::Replace($content, '<script[^>]*>[\s\S]*?sigma[\s\S]*?</script>', '', [System.Text.RegularExpressions.RegexOptions]::IgnoreCase)
...
foreach ($file in $htmlFiles) {
    ...
    $hasLibraries = $content -match "marked\.min\.js"
    
    if ($hasFavicon -and $hasSigmaAI -and $hasLibraries) {
        $completeCount++
    } else {
        $problemCount++
    }
...
Write-Host "`nRESULTADOS FINALES:" -ForegroundColor Magenta
Write-Host "Archivos completos: $completeCount" -ForegroundColor Green
Write-Host "Archivos con problemas: $problemCount" -ForegroundColor Red
Write-Host "Porcentaje de exito: $([math]::Round(($completeCount / $htmlFiles.Count) * 100, 2))%" -ForegroundColor Cyan

if ($completeCount -eq $htmlFiles.Count) {
    Write-Host "`n🎉 EXITO TOTAL! Todos los archivos estan correctos." -ForegroundColor Green
} else {
    Write-Host "`n⚠️ Algunos archivos necesitan revision." -ForegroundColor Yellow
}
